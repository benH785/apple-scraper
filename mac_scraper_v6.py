...
GOOGLE_SHEET_NAME = "Apple Mac Products V6"  # Updated sheet name
...
class AppleMacScraperV6:
    ...
    def extract_products_with_prices_from_category_page(self, soup: BeautifulSoup) -> List[Dict]:
        ...
        for i, section in enumerate(sections):
            try:
                ...
                price_text = section[url_end + 1:].strip()
    ...
    def extract_prices_from_text(self, price_text: str) -> Dict:
        """Extract pricing information from Apple's text format."""
        prices = {
            'current_price': None,
            'original_price': None,
            'savings': None,
        ...
        try:
            ...
            # Pattern 1: Three prices "£XXX.XX £YYY.YY £ZZZ.ZZ" (current, original, savings)
            three_price_pattern = re.findall(r'£([\d,]+\.?\d*)', clean_text)
            if len(three_price_pattern) >= 3:
                try:
                    current = float(three_price_pattern[0].replace(',', ''))
                    original = float(three_price_pattern[1].replace(',', ''))
                    savings = float(three_price_pattern[2].replace(',', ''))
                    
                    # Validate that the math makes sense
                    if abs((original - current) - savings) < 1:  # Allow for rounding
            ...
            # Pattern 2: Two prices "£XXX.XX £YYY.YY" 
            if len(three_price_pattern) >= 2:
                try:
                    price1 = float(three_price_pattern[0].replace(',', ''))
                    price2 = float(three_price_pattern[1].replace(',', ''))
            ...
            # Pattern 3: Single price "£XXX.XX"
            if len(three_price_pattern) >= 1:
                try:
                    price = float(three_price_pattern[0].replace(',', ''))
                    if 300 <= price <= 10000:  # Reasonable Mac price range
            ...
            print(f"❌ Error extracting prices from text '{price_text[:30]}...': {e}")
