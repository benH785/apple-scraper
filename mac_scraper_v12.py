...
class AppleMacScraperV6:
    ...
    def extract_products_with_prices_from_category_page(self, soup: BeautifulSoup) -> List[Dict]:
        """
        BACK TO BASICS: Use the original working approach but extract prices from category page.
        """
        products = []
        
        # Find all product links (this worked in V5)
        ...
        for i, link in enumerate(product_links):
            try:
                ...
                # NEW APPROACH: Extract price from the category page around this link
    ...
    def extract_prices_from_text(self, price_text: str) -> Dict:
        """Extract pricing information from text - simplified and more robust."""
        prices = {
            'current_price': None,
            'original_price': None,
            'savings': None,
    ...
    def extract_products_split_method(self, soup: BeautifulSoup) -> List[Dict]:
        """Fallback method using split-based parsing."""
        products = []
        ...
        print(f"🔄 Split method: Found {len(sections)} potential product sections")
        ...
        for i, section in enumerate(sections):
            try:
                ...
                # Get just the immediate price text after the closing parenthesis
                remaining_text = section[url_end + 1:]
                # Find the first occurrence of a price pattern
                price_match = re.search(r'([^[]*?)(?:\s*-|$)', remaining_text)
